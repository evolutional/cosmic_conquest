...
int klingon(int pla) {
  struct NCommand c;
  int i, j, k, shi, sta, sta2, tas, p, dis;
  ...
  for (i = 0; i < PLAYERNO; i++)
    if (i != pla && general.relation[i][pla] == WAR &&
        general.relation[pla][i] != WAR) {
      c.par[0] = i;
      do_command(pla, &c);
    }
  ...
  for (tas = 0; tas < TASKNO; tas++) {
    ...
    for (i = 0; i < PLAYERNO; i++)
      if (i != pla && search_task(i, sta) != TASKNO &&
          general.relation[i][pla] == PEACE) {
        c.com = DECLARE;
        c.par[0] = i;
        c.par[1] = WAR;
        do_command(pla, &c);
  ...
  for (tas = 0; tas < TASKNO; tas++) {
    ...
    c.par[0] = tas;
    for (p = 0; p < PLANETNO; p++) {
      if (task[pla][tas].ship_free[POPUNIT] +
              task[pla][tas].ship_free[OREUNIT] ==
          0)
        continue;
      if (star[sta].planet[p].ruler != pla &&
          star[sta].planet[p].population > 0)
        continue;
      if (star[sta].planet[p].size == 0)
        continue;
      if (star[sta].planet[p].size <= star[sta].planet[p].industry)
        continue;
      c.par[1] = p;
      do_command(pla, &c);
    }
    ...
    for (p = 0; p < PLANETNO; p++)
      if (star[sta].planet[p].ruler == pla &&
        ...
        if (star[sta].planet[p].mineable) {
          c.par[2] = MINING;
          if (star[sta].planet[p].producing != c.par[2])
            do_command(pla, &c);
          continue;
        }
        ...
        if (star[sta].planet[p].industry +
                star[sta].planet[p].industry / ICOST <
            star[sta].planet[p].population) {
          c.par[2] = INVESTING;
          if (star[sta].planet[p].producing != c.par[2])
            do_command(pla, &c);
          continue;
    ...
    if ((sta = task[pla][tas].headed) != -1 &&
      ...
      while (fi_pres(pla, sta) > fi_need(pla, sta)) {
        ...
        while (j < i && shi != SHIPNO)
          for (shi = 0; shi < SHIPNO; shi++)
            if (task[pla][tas].ship_free[shi] > c.par[shi + 1] &&
                ship[shi].hyprange >= dis && fi_value(shi) <= k &&
                fi_value(shi) != 0) {
              c.par[shi + 1]++;
              j += fi_value(shi);
              k -= fi_value(shi);
    ...
    if ((sta = task[pla][tas].headed) != -1 &&
      ...
      while (pop_pres(pla, sta) > pop_need(pla, sta)) {
        ...
        while (i > j && shi != SHIPNO)
          for (shi = 0; shi < SHIPNO; shi++)
            if (task[pla][tas].ship_free[shi] > c.par[shi + 1] &&
                ship[shi].hyprange >= dis &&
                (k >= fi_value(shi) || fi_value(shi) <= 0) &&
                ship[shi].capacity > 0) {
              c.par[shi + 1]++;
              j += ship[shi].capacity;
              k -= fi_value(shi);
...
int fi_send(int pla, int sta) {
  ...
  for (tas = 0; tas < TASKNO; tas++)
    if (task[pla][tas].headed == sta)
      for (shi = 0; shi < SHIPNO; shi++)
        send += task[pla][tas].ship_free[shi] * fi_value(shi);
  return (send);
}
...
int best_fi(int pla, int sta) {
  ...
  b_val = 0;
  for (shi = 0; shi < SHIPNO; shi++)
    if (ship[shi].hyprange >= dis &&
        ship[shi].techlevel <= general.techlevel[pla] &&
...
int tr_pres(int pla, int sta) {
  ...
  for (tas = 0; tas < TASKNO; tas++)
    if (ATSTAR(pla, tas, sta))
      pres += task_capa(pla, tas);
  return (pres);
}

int tr_send(int pla, int sta) {
  int send = 0;
  for (int tas = 0; tas < TASKNO; tas++)
    if (task[pla][tas].headed == sta)
      send += task_capa(pla, tas);
  return (send);
}

int tr_value(int shi) { return (ship[shi].hypspeed * ship[shi].capacity); }
...
int pop_pres(int pla, int sta) {
  ...
  for (int tas = 0; tas < TASKNO; tas++)
    if (ATSTAR(pla, tas, sta))
      pres +=
          task[pla][tas].ship_free[POPUNIT] + task[pla][tas].ship_load[POPUNIT];
  return (pres);
}
...
int pop_need(int pla, int sta) {
  int need;
  need = 0;
  for (int p = 0; p < PLANETNO; p++)
    if ((star[sta].planet[p].ruler == pla &&
         star[sta].planet[p].poptype == pla) ||
...
int pop_send(int pla, int sta) {
  ...
  for (tas = 0; tas < TASKNO; tas++)
    if (task[pla][tas].headed == sta)
      send +=
          task[pla][tas].ship_load[POPUNIT] + task[pla][tas].ship_free[POPUNIT];
  return (send);
}
...
int ore_pres(int pla, int sta) {
  ...
  for (tas = 0; tas < TASKNO; tas++)
    if (ATSTAR(pla, tas, sta))
      pres +=
          task[pla][tas].ship_free[OREUNIT] + task[pla][tas].ship_load[OREUNIT];
  return (pres);
}
