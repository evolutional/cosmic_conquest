...
long Revision = 35;
struct NStar star[STARNO] = {0};
struct NTask task[PLAYERNO][TASKNO + 1] = {0};
struct NCommData CommData[PLAYERNO] = {0};
struct NGeneral general = {0};
...
conquest() {
  ...
  do {
    ...
    for (pla = 0; pla < PLAYERNO; pla++) {
      cprint(PLAYERNO, "COSMIC CONQUEST", year, "It is the turn of",
             general.pname[pla], "");
      wait_space(pla);
...
load(str) char *str;
{
  ...
  for (pla = 0; pla < PLAYERNO; pla++)
    for (tas = 0; tas < TASKNO; tas++)
      fread(&task[pla][tas], sizeof(struct NTask), 1, file);
  for (pla = 0; pla < PLAYERNO; pla++)
    fread(&CommData[pla], sizeof(struct NCommData), 1, file);
  fclose(file);
  file = NULL;
  return (1);
}
save(str) char *str;
{
  ...
  for (pla = 0; pla < PLAYERNO; pla++)
    for (tas = 0; tas < TASKNO; tas++)
      fwrite(&task[pla][tas], sizeof(struct NTask), 1, file);
  for (pla = 0; pla < PLAYERNO; pla++)
    fwrite(&CommData[pla], sizeof(struct NCommData), 1, file);
  fclose(file);
  file = NULL;
  return (1);
}
init() {
  ...
  wait_space(PLAYERNO);
  randomize();
  cprint(PLAYERNO, "COSMIC CONQUEST", "", "Do you wish to ...",
         "1.Start a new game  ", "2.Restore an old one");
  while ((c = coget()) < '1' || c > '2')
    ;
  if (c == '1') {
    ...
    for (i = 0; i < PLAYERNO; i++) {
      ...
      for (j = 0; j < i; j++)
        if (!strcmp(general.pname[j], general.pname[i]))
          break;
    ...
    cprint(PLAYERNO, "COSMIC CONQUEST", "", "Please wait while",
           "creating the galaxy", "");
    for (i = 0; i < STARNO; i++)
      for (j = 0; j < PLANETNO; j++) {
    ...
    for (i = 0; i < (11 * PLANETNO * STARNO) / 100; i++)
      star[RANDOM(STARNO)].planet[RANDOM(PLANETNO)].size = 10;
    for (i = 0; i < (9 * PLANETNO * STARNO) / 100; i++)
      star[RANDOM(STARNO)].planet[RANDOM(PLANETNO)].size = 20;
    for (i = 0; i < (7 * PLANETNO * STARNO) / 100; i++)
      star[RANDOM(STARNO)].planet[RANDOM(PLANETNO)].size = 40;
    for (i = 0; i < (4 * PLANETNO * STARNO) / 100; i++)
      star[RANDOM(STARNO)].planet[RANDOM(PLANETNO)].size = 60;
    for (i = 0; i < (3 * PLANETNO * STARNO) / 100; i++)
      star[RANDOM(STARNO)].planet[RANDOM(PLANETNO)].size = 120;
    for (i = 0; i < (10 * PLANETNO * STARNO) / 100; i++)
      star[RANDOM(STARNO)].planet[RANDOM(PLANETNO)].mineable = 1;
    spread_stars();
    while (spread_players())
      spread_stars();
    for (i = 0; i < PLAYERNO; i++) {
      general.techlevel[i] = 100;
      for (j = 0; j < PLAYERNO; j++)
        general.relation[i][j] = PEACE;
      for (j = 1; j <= TASKNO; j++) {
        task[i][j].headed = -1;
        task[i][j].arriving = 0;
        for (k = 0; k < SHIPNO; k++) {
...
deinit() {
  ...
  version[4] = '0' + (Revision / 100) % 10;
  version[3] = '0' + (Revision / 1000) % 10;
  cprint(PLAYERNO, "COSMIC CONQUEST", version, "by Carl Edman", "(C) 1988",
         "GOODBYE !");
  wait_space(PLAYERNO);
  return 0;
}
task_speed(pla, tas) int pla;
int tas;
{
  int shi, k = 10000;
  for (shi = 0; shi < SHIPNO; shi++)
    if (task[pla][tas].ship_free[shi])
      k = MIN(k, ship[shi].hypspeed);
  return (k);
}
task_range(pla, tas) int pla;
int tas;
{
  int shi, k = 10000;
  for (shi = 0; shi < SHIPNO; shi++)
    if (task[pla][tas].ship_free[shi])
      k = MIN(k, ship[shi].hyprange);
  return (k);
}
task_capa(pla, tas) int pla;
int tas;
{
  int shi, k = 0;
  for (shi = 0; shi < SHIPNO; shi++)
...
int sta;
{
  ...
  for (p = 0; p < PLANETNO; p++)
    if (star[sta].planet[p].ruler == pla || star[sta].planet[p].poptype == pla)
      return (1);
  if (search_task(pla, sta) != TASKNO)
    return (1);
  else
    return (0);
}
headedstar(pla, sta) int pla;
int sta;
{
  int tas;
  for (tas = 0; tas < TASKNO; tas++)
    if (task[pla][tas].headed == sta)
      return (1);
  return (0);
}
controlstar(sta) int sta;
{
  int p, pla;
  for (p = 0; p < PLANETNO; p++)
  ...
  for (p++; p < PLANETNO; p++)
    if (star[sta].planet[p].ruler != PLAYERNO &&
        star[sta].planet[p].ruler != pla)
      break;
  if (p != PLANETNO)
    return (FIGHTING);
  return (pla);
}
rule_star(pl1, pl2) int pl1;
int pl2;
{
  int sta, p;
  for (sta = 0; sta < STARNO; sta++)
    for (p = 0; p < PLANETNO; p++)
      if (star[sta].planet[p].ruler == pl1 &&
          star[sta].planet[p].poptype == pl2)
        return (1);
  return (0);
}
search_task(pla, sta) int pla;
int sta;
{
  int tas;
  for (tas = 0; tas < TASKNO; tas++)
    if (ATSTAR(pla, tas, sta))
      return (tas);
  return (TASKNO);
}
clean_task(pla) int pla;
{
  int tas, shi;
  for (tas = 0; tas < TASKNO; tas++) {
...
process_turn() {
  ...
  for (sta = 0; sta < STARNO; sta++)
    for (p = 0; p < PLANETNO; p++)
      if (star[sta].planet[p].size) {
        if (star[sta].planet[p].population == 0 ||
        ...
        gr = growth[p] -
             (4 * star[sta].planet[p].population / star[sta].planet[p].size);
        if (gr < 0)
          gr = 0;
          ...
          if (star[sta].planet[p].poptype == pla) {
            ...
            if (tas != -1)
              set_ships_free(pla, tas, POPUNIT,
                             (((int)star[sta].planet[p].population) -
                              star[sta].planet[p].size) +
                                 task[pla][tas].ship_free[POPUNIT]);
          }
          star[sta].planet[p].population = star[sta].planet[p].size;
        }
        if (star[sta].planet[p].industry > star[sta].planet[p].population)
          star[sta].planet[p].industry = star[sta].planet[p].population;
        ...
        case INVESTING:
          ...
          break;
        case RESEARCHING:
          general.techlevel[pla] += (star[sta].planet[p].ipoints / RCOST) /
                                    (general.techlevel[pla] / 100);
        ...
        case MINING:
          ...
          if (tas != -1)
            set_ships_free(pla, tas, OREUNIT,
                           ((int)task[pla][tas].ship_free[OREUNIT]) +
          ...
          if (tas != -1)
            set_ships_free(pla, tas, pro,
                           ((int)task[pla][tas].ship_free[pro]) +
                               star[sta].planet[p].ipoints /
                                   ship[pro].techcost);
          star[sta].planet[p].ipoints %= ship[pro].techcost;
          break;
        }
        if (star[sta].planet[p].industry < 5)
...
playon() {
  ...
  for (pla2 = pla + 1; pla2 < PLAYERNO; pla2++)
    if (exists(pla2))
      break;
  if (pla2 == PLAYERNO) {
    cprint(PLAYERNO, "COSMIC CONQUEST", "GAME OVER", general.pname[pla],
           "has won the galaxy", "");
    wait_space(PLAYERNO);
    return (0);
  }
  cprint(PLAYERNO, "COSMIC CONQUEST", "", "Do you wish to ...", "1.Save Game",
         "2.Don't save");
  while ((c = coget()) < '1' || c > '2')
    ;
  if (c == '1') {
  ...
  cprint(PLAYERNO, "COSMIC CONQUEST", "", "Do you wish to ...", "1.Quit",
         "2.Play on");
  while ((c = coget()) < '1' || c > '2')
    ;
  return (c == '2');
}
distance(sta1, sta2) int sta1;
int sta2;
{
  int r, q;
  if (sta1 < 0 || sta1 >= STARNO || sta2 < 0 || sta2 >= STARNO)
    return (1000);
...
struct NCommand *co;
{
  ...
  case LAND: /* [0]=task [1]=planet */
    if (mode2)
      printf("Pla %d:LAND Task %d Planet %d\n", pla, c0, c1);
    ...
    c2 = MIN(INTASK(pla, c0, POPUNIT),
             star[i].planet[c1].size - star[i].planet[c1].population);
    c3 = MIN(INTASK(pla, c0, OREUNIT),
             star[i].planet[c1].population + c2 - star[i].planet[c1].industry);
    if (star[i].planet[c1].size == 0) {
      nprint(pla, "No such planet.");
      return (-1);
    }
    if (star[i].planet[c1].ruler != pla &&
        star[i].planet[c1].ruler != PLAYERNO && c2 > 0) {
      nprint(pla, "Planet under alien control.");
      return (-1);
    }
    if (star[i].planet[c1].poptype != pla &&
        star[i].planet[c1].population > 0 &&
        star[i].planet[c1].poptype != PLAYERNO && c2 > 0) {
    ...
    if (c2 < 0)
      set_ships_load(pla, c0, POPUNIT,
                     ((int)task[pla][c0].ship_load[POPUNIT]) + c2);
    star[i].planet[c1].industry += c3;
    c3 = task[pla][c0].ship_free[OREUNIT] - c3;
    set_ships_free(pla, c0, OREUNIT, c3);
    if (c3 < 0)
      set_ships_load(pla, c0, OREUNIT,
                     ((int)task[pla][c0].ship_load[OREUNIT]) + c3);
    clean_task(pla);
    update(i);
    nprint(pla, "OK.");
    return (1);
  case CARRY: /* [0]=task [1]=ship of type 0........*/
    if (mode2)
      printf("Pla %d:CARRY Task %d POPUNIT:%d OREUNIT:%d\n", pla, c0,
             co->par[POPUNIT + 1], co->par[OREUNIT + 1]);
    if (task[pla][c0].headed == -1) {
      nprint(pla, "No such taskforce.");
      return (-1);
    }
    ...
    for (i = 0; i < SHIPNO; i++)
      if (co->par[i + 1] != task[pla][c0].ship_load[i])
        break;
    if (i == SHIPNO) {
    ...
    for (i = 0; i < SHIPNO; i++)
      k +=
          INTASK(pla, c0, i) * ship[i].capacity - co->par[i + 1] * ship[i].size;
    if (k < 0) {
  ...
  case DESTINATION: /* [0]=task [1]=star */
    if (mode2)
      printf("Pla %d:DESTINATION Task %d Star %d\n", pla, c0, c1);
    if (task[pla][c0].headed == -1) {
      nprint(pla, "No such taskforce.");
      return (-1);
    }
  ...
  case EMIGRATION: /* [0]=star [1]=planet [2]=amount */
    if (mode2)
      printf("Pla %d:EMIGRATION Star %d Planet %d Amount %d\n", pla, c0, c1,
             c2);
    ...
    set_ships_free(pla, i, POPUNIT,
                   ((int)task[pla][i].ship_free[POPUNIT]) + c2);
  ...
  case JOIN: /* [0]=task1 [1]=task2 ...*/
    ...
    for (i = 0; co->par[i] != TASKNO; i++)
      if (task[pla][co->par[i]].arriving > general.time)
        break;
    if (co->par[i] != TASKNO) {
      nprint(pla, "Taskforce in hyperspace.");
      return (-1);
    ...
    for (i = 0; co->par[i] != TASKNO; i++) {
      for (j = 0;; j++)
        if (co->par[j] == TASKNO || (j != i && co->par[i] == co->par[j]))
          break;
      if (co->par[j] != TASKNO)
        break;
    }
    ...
    for (i = 1; co->par[i] != TASKNO; i++) {
      for (j = 0; j < SHIPNO; j++) {
        set_ships_free(pla, c0, j,
                       ((int)task[pla][c0].ship_free[j]) +
                           task[pla][co->par[i]].ship_free[j]);
        set_ships_load(pla, c0, j,
                       ((int)task[pla][c0].ship_load[j]) +
                           task[pla][co->par[i]].ship_load[j]);
      }
  ...
  case KILL: /* [0]=star [1]=planet*/
    if (mode2)
      printf("Pla %d:KILL Star %d Planet %d\n", pla, c0, c1);
    if (c1 == PLANETNO) {
      j = SHIPNO;
      for (i = 0; i < TASKNO && j == SHIPNO; i++)
        if (ATSTAR(pla, i, c0))
          for (j = 0; j < SHIPNO; j++)
            if ((ship[j].special & STARKILLER) && task[pla][i].ship_free[j])
              break;
  ...
  case SPLIT: /* [0]=task [1]=ship of type 0........*/
    if (mode2)
      printf("Pla %d:SPLIT Task %d POPUNITS %d OREUNITS %d\n", pla, c0,
             co->par[POPUNIT + 1], co->par[OREUNIT + 1]);
    if (task[pla][c0].headed == -1) {
      nprint(pla, "No such taskforce.");
      return (-1);
  ...
  case PRODUCE: /* [0]=star [1]=planet [2]=what */
    if (mode2)
      printf("Pla %d:PRODUCE Star %d Planet %d Product %d\n", pla, c0, c1, c2);
    if (star[c0].planet[c1].size == 0) {
      nprint(pla, "No such planet.");
      return (-1);
    }
    ...
    if (star[c0].planet[c1].producing != c2)
      star[sta].planet[c1].ipoints =
          MIN(star[sta].planet[c1].ipoints, star[sta].planet[c1].industry - 1);
    star[c0].planet[c1].producing = c2;
    nprint(pla, "OK.");
    return (1);
  case DECLARE: /* [0]=on whom [1]=what */
    if (mode2)
      printf("Pla %d:DECLARE DecPla %d Status %d\n", pla, c0, c1);
    if (c0 == pla) {
  ...
  case POSSESS: /* [0]=star [1]=planet */
    if (mode2)
      printf("Pla %d:POSSESS Star %d Planet %d\n", pla, c0, c1);
    ...
    for (i = 0; i < PLAYERNO; i++)
      if (i != pla && general.relation[i][pla] != ALLIANCE &&
          search_att_task(i, c0) != TASKNO)
        break;
    if (i != PLAYERNO) {
    ...
    if (general.techlevel[star[c0].planet[c1].ruler] > general.techlevel[pla])
      general.techlevel[pla] += MIN(
          general.techlevel[star[c0].planet[c1].ruler] - general.techlevel[pla],
          star[c0].planet[c1].industry / 2);
  ...
  default:; /*ERROR*/
    if (mode2)
      printf("Pla %d:ERROR %d\n", pla, co->com);
    return (-1);
  }
}
display_check(pla) int pla;
{
  if (waittime > general.time)
    return (0);
  if (pla == PLAYERNO || mode2 || mode3)
    return (1);
  if (pla == -1)
    return (0);
  if (comp_cmp("klingon", general.pname[pla]))
    return (0);
  if (comp_cmp("romulan", general.pname[pla]))
    return (0);
  if (comp_cmp("alien", general.pname[pla]))
    return (0);
  if (comp_cmp("berserker", general.pname[pla]))
    return (0);
  if (comp_cmp("dead", general.pname[pla]))
    return (0);
  return (1);
}
fight(sta) int sta;
{
  int tas, shi, pla1, pla2, i, j, disp;
  char c[] = " ";
  for (pla1 = 0; pla1 < PLAYERNO; pla1++) {
    if (search_att_task(pla1, sta) == TASKNO)
      continue;
    for (pla2 = 0; pla2 < PLAYERNO; pla2++)
      if (search_task(pla2, sta) != TASKNO &&
          general.relation[pla1][pla2] == WAR)
        break;
    if (pla2 < PLAYERNO)
      break;
  }
  if (pla1 == PLAYERNO)
    return (0);
  for (pla2 = 0; pla2 < PLAYERNO; pla2++) {
    if (!knowstar(pla2, sta))
      continue;
    if (display_check(pla2))
      break;
  }
  if (pla2 < PLAYERNO)
    disp = PLAYERNO;
  ...
  for (pla1 = 0; pla1 < PLAYERNO; pla1++) {
    ...
    for (tas = 0; tas < TASKNO; tas++)
      if (ATSTAR(pla1, tas, sta)) {
        for (shi = 0; shi < SHIPNO; shi++) {
          set_ships_free(pla1, TASKNO, shi,
                         ((int)task[pla1][TASKNO].ship_free[shi]) +
                             task[pla1][tas].ship_free[shi]);
          set_ships_load(pla1, TASKNO, shi,
                         ((int)task[pla1][TASKNO].ship_load[shi]) +
                             task[pla1][tas].ship_load[shi]);
        }
    ...
    for (shi = 0; shi < SHIPNO; shi++)
      if (ship[shi].special & FASTEJECTER)
        i += task[pla1][TASKNO].ship_free[shi] * ship[shi].capacity;
    for (shi = 0; shi < SHIPNO && i > 0; shi++) {
      j = 0;
      if (ship[shi].special & FASTEJECTABLE)
        j = MIN(i / ship[shi].size, task[pla1][TASKNO].ship_load[shi]);
  ...
  for (pla1 = 0; pla1 < PLAYERNO; pla1++)
    if (popore_task(pla1, TASKNO)) {
      ...
      task[pla1][TASKNO].ship_free[POPUNIT] =
          task[pla1][TASKNO].ship_load[POPUNIT] = 0;
      task[pla1][TASKNO].ship_free[OREUNIT] =
          task[pla1][TASKNO].ship_load[OREUNIT] = 0;
      for (pla2 = 0; pla2 < PLAYERNO; pla2++)
        if (att_task(pla2, TASKNO) && general.relation[pla2][pla1] == WAR)
          set_ships_free(pla2, TASKNO, OREUNIT,
                         task[pla2][TASKNO].ship_free[OREUNIT] + i);
    }
  for (pla1 = 0; pla1 < PLAYERNO; pla1++) {
...
fight_now(disp) int disp;
{
  ...
  for (pla1 = 0; pla1 < PLAYERNO; pla1++) {
    platar[pla1] = 0;
    for (pla2 = 0; pla2 < PLAYERNO; pla2++)
      if (general.relation[pla1][pla2] == WAR ||
          general.relation[pla2][pla1] == WAR)
        platar[pla1] += task_size_po(pla2, TASKNO);
  }
  for (cspeed = 0; cspeed < MAXSPEED; cspeed++)
    for (shi1 = 0; shi1 < SHIPNO; shi1++) {
      if (ship[shi1].attackno == 0 || ship[shi1].speed != cspeed)
        continue;
      pl = RANDOM(PLAYERNO);
      last = x;
      for (pla1 = 0; pla1 < PLAYERNO; pla1++)
        plashi[pla1] = task[pla1][TASKNO].ship_free[shi1];
          ...
          for (ano = 0; ano < ship[shi1].attackno; ano++) {
            ...
            if (ship[shi1].special & KAMIKAZE) {
              ...
              for (pla3 = 0; pla3 < PLAYERNO; pla3++)
                if (general.relation[pla3][pla1] == WAR ||
                    general.relation[pla1][pla3] == WAR)
                  platar[pla3] -= ship[shi1].size;
            }
            for (pla3 = 0; pla3 < PLAYERNO; pla3++)
              if (general.relation[pla3][pla2] == WAR ||
                  general.relation[pla2][pla3] == WAR)
                platar[pla3] -= ship[shi2].size;
          }
        }
      }
    }
...
search_att_task(pla, sta) int pla;
int sta;
{
  int tas;
  for (tas = 0; tas < TASKNO; tas++)
    if (ATSTAR(pla, tas, sta) && att_task(pla, tas))
      return (tas);
  return (TASKNO);
}
att_task(pla, tas) int pla;
int tas;
{
  int shi;
  if (task[pla][tas].headed == -1)
...
popore_task(pla, tas) int pla;
int tas;
{
  int shi;
  for (shi = 0; shi < SHIPNO - 2; shi++)
    if (INTASK(pla, tas, shi))
      return (0);
  return (1);
}
exists(pla) int pla;
{
  ...
  for (s = 0; s < STARNO; s++)
    for (p = 0; p < PLANETNO; p++)
      if (star[s].planet[p].poptype == pla && star[s].planet[p].ruler == pla &&
          star[s].planet[p].population > 0)
        return (1);
  for (t = 0; t < TASKNO; t++)
    if (task[pla][t].headed != -1 && INTASK(pla, t, POPUNIT))
      return (1);
  return (0);
}
population_no(pla) int pla;
{
  int i = 0, s, p;
  for (s = 0; s < STARNO; s++)
    for (p = 0; p < PLANETNO; p++)
      if (star[s].planet[p].poptype == pla)
        i += star[s].planet[p].population;
  return (i);
}
industry_no(pla) int pla;
{
  int i = 0, s, p;
  for (s = 0; s < STARNO; s++)
    for (p = 0; p < PLANETNO; p++)
...
nearstar(sta) int sta;
{
  int sta1 = -1, dis, j;
  dis = 1000;
  for (j = 0; j < STARNO; j++)
    if (j != sta && distance(sta, j) < dis) {
      sta1 = j;
      dis = distance(sta, sta1);
    }
  return (sta1);
}
set_ships_free(pla, tas, shi, no) int pla;
int tas;
int shi;
int no;
{
...
set_ships_load(pla, tas, shi, no) int pla;
int tas;
int shi;
int no;
{
...
spread_players() {
  ...
  for (i = 0; i < PLAYERNO; i++) {
    ...
    for (l = 0; l < 100 && task[i][0].headed == -1; l++) {
      j = RANDOM(STARNO);
      if (distance(j, nearstar(j)) > 4)
        continue;
      for (k = 0; k < i; k++)
        if (task[k][0].headed == j || distance(task[k][0].headed, j) < 8)
          break;
      if (k == i)
        task[i][0].headed = j;
    }
...
update(sta) int sta;
{
  int p, pla, ruler, totsize, totpop, totind;
...
int sta;
{
  ...
  for (pl = 0; pl < PLAYERNO; pl++)
    if (pl != pla)
      ...
      case PEACE:
        danger += fi_pres(pl, sta);
        break;
      case WAR:
        danger += fi_pres(pl, sta) * 2;
        break;
      case ALLIANCE:
        danger += fi_pres(pl, sta) / 2;
        break;
...
int sta;
{
  ...
  for (tas = 0; tas < TASKNO; tas++)
    if (ATSTAR(pla, tas, sta))
      pres += fi_strength(pla, tas);
  return (pres);
}
fi_strength(pla, tas) int pla;
int tas;
{
  int shi, str;
  str = 0;
  for (shi = 0; shi < SHIPNO; shi++)
    str += task[pla][tas].ship_free[shi] * fi_value(shi);
  return (str);
}
fi_value(shi) int shi;
{
  if (ship[shi].attack * ship[shi].attackno == 0)
    return (0);
  return (ship[shi].attack * ship[shi].attackno +
          ship[shi].shields * ship[shi].shields / 5);
}
